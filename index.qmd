...
**NOTE: Results are final for Week `r as.numeric(readr::read_lines("dat/update_week.txt"))`, but Sleeper does not like to share scores for teams not in Champion or Toilet bracket matchups. Eliminator tables updated by hand herein, other tables on this site may contain inaccuracies past the regular season (Week 14).**
...
Bylaws for the league are available here, reach out to LTS commish Greg Bryant by [email](mailto:lts.leaguecommish@gmail.com) or on [Discord](https://discord.com/channels/@me/1142996811498397816) for access: [2025 bylaws](https://docs.google.com/document/d/157aW2NfX060KScasvEXd19t5CWQ6193_MxJabeilkjk/edit?usp=sharing)
...
df_all_transactions <- purrr::map(lts_conn, ~ ff_transactions(.x)) %>%
  ...
  mutate(
    week = difftime(timestamp, lubridate::ymd("2025-09-03"), units = "weeks") %>% ceiling(.) %>% as.integer(.) %>% if_else(. < 0L, 0L, .),
